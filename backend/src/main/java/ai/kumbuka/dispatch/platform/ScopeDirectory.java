    ...
    private void requireServedKind(ScopeAccess scope) {
        if (KIND_PRIVATE.equals(scope.kind())) {
            ...
            throw new DispatchException(DispatchException.Reason.SCOPE_KIND_UNSUPPORTED,
                ...
                // refused. It is a VALUE for the message pattern and never
                // reaches `data`: the surface reads it and drops it. Without
                // this the adapters would each hold their own copy of "the
                // refused kind is private", and a second refused kind would
                // silently keep wording itself as the first.
                List.of(scope.kind()));
        }
    }
