...
its answer and refusal shapes follow DEC-0040 and DEC-0042. The tool descriptions in section 5 are
normative text: the conformance probe of the service and of the router takes its expected values
...
An **addendum** is a correction attached to a frozen exchange, addressed with a letter after the
sub-position (`.3a`). It is not a child and closes with the exchange it corrects.
...
Two roles take part. The **commissioner** gives and accepts work and must be a console identity. The
**executor** takes up and delivers work, holds the exchange through a claim, and proves it with the
receipt the claim returned. One identity may act in both roles on different exchanges, never both on
the same one.
...
object's state and the caller's entitlement (section 6). Where it is empty and the exchange is not
finished, `waiting_for` names who or what the exchange waits for. A listing carries the same two
members on every entry.
...
name, including an argument nested in `fields` (REQ-0113). Measured on 2026-09-18: `create`
accepted `draft` inside its body, discarded it and answered with success.
...
- It names the call the caller made, under that name. Kernel names never appear.
...
| `ROLE_DOES_NOT_ALLOW` | `<call> can only be made by the <role>. You take part in <address> as <role>.` | the calls in `data.next` |
| `NOT_THE_HOLDER` | `<address> is held by someone else until <time>. Only the holder can <what the call does>.` | wait, or `dispatch_read` |
| `RECEIPT_MISSING` / `RECEIPT_WRONG` | `<call> needs the receipt you received from dispatch_take for <address>.` / `... the receipt given is not the one issued for <address>.` | pass the receipt from the take |
| `NO_ANSWER_DELIVERED` | `<address> has no delivered answer to accept. The holder delivers with dispatch_deliver_return.` | wait for the executor |
| `CHILDREN_NOT_FINISHED` | `<root> cannot be closed while <n> exchange(s) of the bracket are unfinished: <address> (<state>), ...` with `data.offenders` as `[{ address, state, next }]` | the calls in each offender's `next` |
| `NOTHING_TO_TAKE` | `Nothing in dispatch://<scope>/<selector> can be taken up right now: every exchange is finished, held by someone, or waiting for its commissioner.` | `dispatch_query` |
...
| `SELECTOR_UNKNOWN` | `<selector> is not a bracket kind declared in scope <scope>. Declared: <list>.` | use a declared one |
| `UNEXPECTED_FAILURE` | `<call> on <address> failed unexpectedly. This is a defect, not a rule. Nothing was changed. Report reference <ref>.` | report the reference |
...
Each entry gives the tool name, its description (normative), its arguments, the role, the effect on
the kernel and the resulting state. Arguments follow DEC-0040: those that choose the target and the
transport artefacts are top-level; everything the call writes is under `fields`.
...
optional `metadata`. Role: commissioner. Kernel: create, write the dispatch body, send, in one
...
> the exchange, cannot be removed, and closes together with it.

Top-level: `address`, optional `idempotency_key`. Under `fields`: `title`, `text`. Role:
commissioner. Kernel: append. Result: the exchange, unchanged in state.
...
Top-level: `address`. Role: commissioner, never the executor of this exchange. From: `needs_input`
with a delivered answer, or `returned`. Kernel: ratify and close in one transaction. Result: `closed`.

**`dispatch_curate_return`**
> Accept the delivered answer and finish the exchange by carrying it forward into a named object,
> for example the record of the bracket it belongs to. The target is stored with the exchange.

Top-level: `address`. Under `fields`: `into`, the complete address of the target. Role and
from-states as `dispatch_accept_return`. Kernel: ratify and consume in one transaction; `into` is
resolved and stored as the target's durable identity (ADR-0014). Result: `consumed`.
...
Top-level: `address`, `conflict_token`. Under `fields`: `message`. Role: commissioner. From:
...
> Not for a bracket root; a bracket is finished with dispatch_close_bracket.

Top-level: `address`, `conflict_token`. Under `fields`: `reason`. Role: commissioner. From: `open`,
`active`, `needs_input`. Kernel: store the reason, close. Result: `closed`.
...
Top-level: `address` of the root. Role: commissioner. From: root in `needs_input` with a delivered
...
Top-level: `address`, `duration`. Kernel: claim. Result: `active`, plus `receipt`.

**`dispatch_take_next`**
> Take up the next open exchange of a bracket kind, in address order. Returns the exchange and the
> receipt.

Top-level: `scope`, `selector`, `duration`. Kernel: claim_next. Result: `active`, plus `receipt`.
...
Top-level: `address`, `receipt`. Under `fields`: `text`. Role: holder. From: `active`. Kernel: write
...
Top-level: `address`, `receipt`. Under `fields`: `question`. Role: holder. From: `active`. Kernel:
store the question, block. Result: `needs_input`.

**`dispatch_decline`**
> Decline the work. Before you took it up this records a refusal of the commission; after, a failure
> to complete it. The reason is stored. Final.

Top-level: `address`, and `receipt` if taken up. Under `fields`: `reason`. Kernel: abandon. Result:
`rejected` or `failed`.
...
|---|---|---|---|
| `open` | commissioner | `dispatch_add_correction`, `dispatch_cancel` | the executor |
| `open` | anyone else | `dispatch_take` | -- |
| `active` | holder | `dispatch_deliver_return`, `dispatch_ask_commissioner`, `dispatch_decline` | -- |
| `active` | commissioner | `dispatch_add_correction`, `dispatch_cancel` | the executor |
| `needs_input`, answer delivered | commissioner | `dispatch_accept_return`, `dispatch_curate_return`, `dispatch_reply_to_executor` (root: `dispatch_close_bracket` instead of the first two) | -- |
| `needs_input`, question asked | commissioner | `dispatch_reply_to_executor`, `dispatch_cancel` | -- |
| `needs_input` | holder | (none) | the commissioner |
| `returned` | commissioner | `dispatch_accept_return`, `dispatch_curate_return` (root: `dispatch_close_bracket`) | -- |
| any terminal | anyone | (none) | nobody: the exchange is finished |

`dispatch_read` is always available on a visible exchange and is not listed. A call C is not
entitled to is never listed.
...
4. `next` and `waiting_for` in every answer and on every listing entry, and `next` inside `data` of
   every refusal that carries `data`, computed from the transition table and the caller.
5. Refusal messages under section 4: surface names instead of kernel names, the way out instead of
   the way in, lists in `data`.
6. Every address in every answer and refusal in complete form.
7. Closed input schemas on the service and on the router: no argument accepted and discarded. The
   router's schema for `create` currently declares `additionalProperties: true` on `body`, which is
   the path by which `draft` reached the service and was dropped there.
8. A catalogue of declared reasons checked at start-up.
9. The generic REST surface returns the same answers and refusals.
...
- The executor's text-bearing verbs rely on the claim and the receipt for exclusivity and take no
  ...
  side is deliberate.
- Whether `dispatch_decline` after take-up checks the receipt or the holder's identity follows the
  lease mechanics and is not settled here.
