 ...
 *
 * <p>It is a file rather than a constant, and it is read rather than derived:
 * a probe that took its expectation from the code would assert that one copy
 * equals itself. The two halves of the probe — closure, read statically from
 * the annotations, and coverage, measured end to end against a running service
 * — read this same file, so they cannot disagree about what was specified even
 * while they disagree about what was built.
 *
 * <p>A reader rather than a test, so both halves can use it and neither owns
 * it. It asserts only about itself: that the file is there, that its rows have
 * the shape they claim, and that it is not empty — an empty expectation passes
...
public final class VerbSurfaceSpecification {
     ...
     * One specified row.
     *
     * @param klass  carried, uncarried, refusal, binding or extension
     * @param verb   the platform verb, or '-' where the row names none
     * @param method the HTTP method
     * @param path   the form, exactly as it must appear
     * @param depth  item, collection, sub-collection or none
     */
